...
class AddOrderDialog(QDialog):
    ...
    def calculate_total_price(self):
        try:
            ...
            total = width * length * price_per_cm
    ...
    def save_order(self):
        ...
        if not client_id or not order_name or not width or not length or not price_per_cm or not total_price:
        ...
        cursor.execute("""
            INSERT INTO orders (order_name, client_id, width, length, price_per_cm, total_price, payment_type, order_date, order_day)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (order_name, client_id, width, length, price_per_cm, total_price, payment_type, order_date_str, arabic_day))
        # Update total bill
