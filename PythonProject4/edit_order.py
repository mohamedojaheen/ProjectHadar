...
class EditOrderDialog(QDialog):
    ...
    def populate_table(self):
        ...
        if self.order_id:
            cursor.execute("""
                SELECT orders.id, clients.name, orders.order_name, orders.width, orders.length, orders.price_per_cm, orders.total_price, orders.payment_type, orders.order_date, orders.order_day
                FROM orders 
        ...
        self.table.setColumnCount(10)  # Including ID column
        self.table.setHorizontalHeaderLabels(["ID", "العميل", "اسم الطلب", "العرض (سم)", "الطول (سم)", "السعر لكل سم", "السعر الإجمالي", "نوع الدفع", "تاريخ الطلب", "اليوم"])
...
class EditOrderDialogEdit(QDialog):
    ...
    def save_order(self):
        ...
        cursor.execute("""
            UPDATE orders
            SET order_name = ?, width = ?, length = ?, price_per_cm = ?, total_price = ?, payment_type = ?
            WHERE id = ?
        """, (self.order_name_input.text(), self.width_input.text(), self.length_input.text(), self.price_per_cm_input.text(), new_total_price, self.payment_type_input.currentText(), self.order_id))
        conn.commit()
        conn.close()
