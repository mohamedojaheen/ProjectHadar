...
class ClientRecordsDialog(QDialog):
    ...
    def populate_orders(self, client_id):
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                SELECT orders.id, orders.order_name, orders.width, orders.length, orders.price_per_cm, orders.total_price, orders.payment_type, orders.order_date, orders.order_day
                FROM orders
                WHERE client_id = ?
            """, (client_id,))
            ...
            self.orders_table.setColumnCount(8)  # orders.id removed, order_day added
            self.orders_table.setHorizontalHeaderLabels(["اسم الطلب", "العرض (سم)", "الطول (سم)", "السعر لكل سم", "إجمالي السعر", "نوع الدفع", "تاريخ الطلب", "اليوم"])
